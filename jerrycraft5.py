...
FOREST_GREEN: Tuple[int, int, int] = (34, 139, 34)  # Grass
...
SIDEBAR_BG: Tuple[int, int, int] = (40, 40, 55)  # Sidebar background
NIGHT_OVERLAY_COLOR: Tuple[int, int, int, int] = (50, 50, 70, 100)  # Night overlay with alpha
...
# Additionally, we define a pause button size/position.
...
class Game:
    ...
    def reward_inventory(self) -> None:
        # Normal reward for surviving full night
    ...
    def reward_inventory_bonus(self) -> None:
        # Bonus reward: award three fire blocks instead of one for bonus night win
        self.inventory["mud"] += 5 + self.round_number
        self.inventory["stone"] += 3 + self.round_number
        self.inventory["water"] += 1 + (self.round_number // 2)
        self.inventory["sand"] += 4 + self.round_number
        self.inventory["fire"] += 3
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.pause_button_rect and self.pause_button_rect.collidepoint(mouse_pos):
                    ...
                    continue  # Skip further processing if pause toggled
    ...
    def update(self) -> None:
        ...
        # Early Transition: If in night phase and all Jeepers are killed, transition immediately
        if self.phase == "night" and not self.game_over and len(self.creepers) == 0:
            # Award bonus: three fire blocks instead of the usual fire reward
            self.phase = "build"
            self.round_number += 1
            self.phase_start_time = now
            self.time_left_in_phase = BUILD_PHASE_DURATION
            ...
            self.creepers = []
            self.transition_message = f"Congratulations! You made it to round {self.round_number}"
            self.transition_message_end = now + 3000  # Show message for 3 seconds
    ...
    def draw_ui(self) -> None:
        ...
        # 0. Pause/Resume Button at the top
        ...
        for line in instructions:
            ...
            inst_y += UI_LINE_HEIGHT * 0.8

        # 6. Transition Message (bonus win)
